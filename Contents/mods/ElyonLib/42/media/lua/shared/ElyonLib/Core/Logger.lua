...
function Logger:new(modID, modVersion)
	if loggers[modID] then
		return loggers[modID]
	end
...
function Logger:writeLog(options)
	...
	elseif logMode == "both" then
		...
		if Logger.isServer then
			...
			if player then
				sendClientCommand(player, "ElyonLib", "LogToServer", { logFileMask = logFileMask, logText = logText })
			else
				self:error("Logger.writeLog could not send LogToServer because no local player exists.")
			end
		end
	end
end
