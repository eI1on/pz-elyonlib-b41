...
local function writeToFile(fileWriter, content, modId)
	...
	if not success then
		...
		logger:error(string.format("Failed to write content: %s", tostring(err)))
	end
...
function FileUtils.readFile(filePath, modId, options)
	...
	local ok, result = pcall(function()
		if options.isModFile then
			return getModFileReader(modId, filePath, options.createIfNull or false)
		end
		return getFileReader(filePath, options.createIfNull or false)
	end)
	if ok then
		fileReader = result
	else
		local logger = Logger:new(modId)
		logger:error(string.format("Failed to open file for reading: %s. Error: %s", filePath, tostring(result)))
	end
...
function FileUtils.writeFile(filePath, content, modId, options)
	...
	local ok, result = pcall(function()
		if options.isModFile then
			return getModFileWriter(modId, filePath, options.createIfNull or false, options.append or false)
		end
		return getFileWriter(filePath, options.createIfNull or false, options.append or false)
	end)
	if ok then
		fileWriter = result
	else
		local logger = Logger:new(modId)
		logger:error(string.format("Failed to open file for writing: %s. Error: %s", filePath, tostring(result)))
	end
...
function FileUtils.readJson(filePath, modId, options)
	...
	if not success then
		local logger = Logger:new(modId)
		logger:error(string.format("Failed to parse json from file: %s. Error: %s", filePath, tostring(data)))
		return nil
	end
	return data
end
...
function FileUtils.writeJson(filePath, data, modId, options)
	...
	if not success then
		local logger = Logger:new(modId)
		logger:error(
			string.format("Failed to serialize data to json for file: %s. Error: %s", filePath, tostring(content))
		)
		return false
	end
